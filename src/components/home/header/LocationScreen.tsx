...
const LocationScreen: React.FC<LocationScreenProps> = ({ onClose, showHeader = true }) => {
  const { setLocation, currentLocation } = useLanguage();
  ...
  const stepConfig = {
    states: {
      ...
      subtitle: "Select your current department."
    },
    cities: {
      question: "Which commune are you located in?",
      subtitle: `Choose your commune in ${selectedState?.name}.`
    },
    sections: {
      question: "Which section communale are you in?",
      subtitle: `Choose your section in ${selectedCity?.name}.`
    },
    quartiers: {
      question: "Which quartier are you in?",
      subtitle: `Choose your quartier in ${selectedSection?.name}.`
    }
  };
      ...
      async (position) => {
        try {
          ...
          if (detectedCountry) {
            ...
            setGeoError('Could not find your location in our database');
          }
        } catch (error) {
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600">
                        Click this icon to automatically detect your location using your device's GPS.
                      </p>
                    </div>
                    <button 
                      onClick={dismissTip}
          ...
          <>
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className={selectedState ? "text-gray-900" : "text-gray-500"}>
                      {selectedState ? selectedState.name : "Choose a department..."}
                    </span>
            ...
            <div>
              <div className="flex items-center justify-center gap-2 mb-3">
                ...
                <h3 className="text-sm font-medium text-gray-700">Popular Departments</h3>
              </div>
        ...
        {locationLevel === 'cities' && (
          <>
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className={selectedCity ? "text-gray-900" : "text-gray-500"}>
                      {selectedCity ? selectedCity.name : "Choose a commune..."}
                    </span>
                    <ChevronDown className="h-4 w-4 text-gray-400" />
                  </div>
                </button>
            ...
            <div>
              <div className="flex items-center justify-center gap-2 mb-3">
                <Star className="h-4 w-4 text-gray-600" />
                <h3 className="text-sm font-medium text-gray-700">Popular Communes</h3>
              </div>
              <div className="flex flex-wrap gap-2 justify-center">
            ...
            <div className="mb-6">
              <div className="relative">
                <button
                  onClick={() => {
                    setLocationListScreenOpen(true, {
                      title: "Choose Section Communale",
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className={selectedSection ? "text-gray-900" : "text-gray-500"}>
                      {selectedSection ? selectedSection.name : "Choose a section communale..."}
                    </span>
                    <ChevronDown className="h-4 w-4 text-gray-400" />
                  </div>
                </button>
        ...
        {locationLevel === 'quartiers' && (
          <>
            <div className="mb-6">
              <div className="relative">
                <button
                  onClick={() => {
                    setLocationListScreenOpen(true, {
                      title: "Choose Quartier",
                      items: filteredQuartiers,
                      onSelect: handleQuartierSelect,
                      searchPlaceholder: "Search quartiers..."
                    });
                  }}
                  className="w-full p-3 text-left bg-white border border-gray-300 rounded-lg hover:border-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-colors"
                >
                  <div className="flex items-center justify-between">
                    <span className={selectedQuartier ? "text-gray-900" : "text-gray-500"}>
                      {selectedQuartier ? selectedQuartier.name : "Choose a quartier..."}
                    </span>
                    <ChevronDown className="h-4 w-4 text-gray-400" />
                  </div>
                </button>
            ...
            <div>
              <div className="flex items-center justify-center gap-2 mb-3">
                <Star className="h-4 w-4 text-gray-600" />
                <h3 className="text-sm font-medium text-gray-700">Popular Quartiers</h3>
              </div>
              <div className="flex flex-wrap gap-2 justify-center">
...
<div className="bg-white p-4 sticky bottom-0"> {/* Removed border classes here */}
  ...
  >
    {locationLevel === 'quartiers' ? 'Terminer' : 'Continuer'}
  </button>
</div>
    </div>
  );
