...
const navItems: BottomNavTab[] = [
  ...
  { id: 'messages', name: 'Messages', icon: MessageCircle, path: '/messages', badge: 5 },
{ id: 'more', name: 'More', icon: LayoutGrid, path: '/more-menu' },
  { id: 'account', name: 'Account', icon: User, path: '/profile' },
  { id: 'videos', name: 'Videos', icon: Tv, path: '/videos' },
  { id: 'notifications', name: 'Notifications', icon: Bell, path: '/notifications', badge: 12 },
  { id: 'bookmarks', name: 'Bookmarks', icon: Bookmark, path: '/bookmarks' },
  { id: 'friends', name: 'Friends', icon: Users, path: '/friends', badge: 3 },
...
export default function BottomNav() {
  ...
  const [selectedMoreItem, setSelectedMoreItem] = useState('More');
  ...
  const handleMoreClick = () => {
    ...
    if (savedSelectedItem && savedSelectedItem !== 'More') {
      const selectedItem = navItems.find(item => item.name === savedSelectedItem);
                ...
                <div className="relative flex items-center justify-center">
                  {item.isAvatar && user ? (
                    ...
                    <div className="flex items-center">
                      {(() => {
                        const selectedItem = navItems.find(navItem => navItem.name === selectedMoreItem);
                        const IconToShow = selectedItem && selectedMoreItem !== 'More' ? selectedItem.icon : LayoutGrid;
                        return (
                          <IconToShow
                            className={cn(
                  ...
                  {isActive && item.name && (
                    <span className="ml-2 font-medium whitespace-nowrap max-w-[80px] overflow-hidden">
                      {item.id === 'more' ? selectedMoreItem : item.name}
                    </span>
                  )}
                </div>
              </button>
